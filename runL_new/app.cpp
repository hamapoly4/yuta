...
namespace {
    ...
    void system_create()
    {
        ...
        for (i = 0; i < SIZE; i++)
        {
            ...
            {
                gRun[i] = new LineTraceRun(RIGHT, HIGH, gLeftMotor, gRightMotor, gPIDcon[HIGH]);
            }
        }
...
{
    if (gScenarioTracer->getState() == gScenarioTracer::FIN || ev3_button_is_pressed(ENTER_BUTTON) == true)
    {
        wup_tsk(MAIN_TASK);
    }
    else
