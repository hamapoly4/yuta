...
void system_create()
{
     ...
     * 制御方法：ラインに沿って走行するための制御(PID)
     * ------------------------------------------------------------*/
    gControl[Control::TO_DOUBLE_SIZE + 3] = new LineTracePID(gHSVMonitor, 40, 0.5, 0.025, 2);
    gRun[Scenario::TO_DOUBLE_SIZE + 5]     = new LineTraceRun(LineTraceRun::LEFT, gControl[Control::TO_DOUBLE_SIZE + 3], gLeftMotor, gRightMotor, Run::LOW);
    gJudge[Scenario::TO_DOUBLE_SIZE + 5]   = new RunDistJudge(gRunDistDetect);
    ...
    gRun[Scenario::TO_DOUBLE_SIZE + 6]     = new LineTraceRun(LineTraceRun::LEFT, gControl[Control::TO_DOUBLE_SIZE + 4], gLeftMotor, gRightMotor, Run::MID);
    ...
    gToGoalScenario = new Scenario(gScene, 26, 32);
